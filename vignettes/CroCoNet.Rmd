...
The function <tt>filterModuleTrees</tt> calculates the probability that the statistic of an actual modules comes from the distribution of the statistics of all random modules (one-sample one-sided z-test with the mean and standard deviation of the random module statistics as the expected mean and standard deviation). If a module is **not** significantly different from the random modules in terms of both statistics, it is removed.

```{r filter_module_trees}
tree_stats_filt <- filterModuleTrees(tree_stats, random_tree_stats)
```

In case of the example dataset, this filtering removes 11 modules (marked in red):

```{r plot_total_vs_within_filt}
plotTreeStats(tree_stats, 
              random_tree_stats, 
              c("within_species_diversity", "total_tree_length")) +
  geom_point(data = anti_join(tree_stats, tree_stats_filt), color = "red3", size = 1.2)
```
...
```{r cons_div_mod, eval = TRUE, include = TRUE, echo = FALSE, out.width = "75%", fig.align = 'center', fig.cap = 'Weighted linear regression between the total tree length and within-species diversity, with the top 4 most conserved and most diverged modules marked.'}
...
```{r get_top4_cons_div}
top4_cons_div_modules <- module_conservation_overall %>% 
  dplyr::filter(conservation != "not_significant") %>% 
  dplyr::arrange(residual) %>% 
  dplyr::slice(1:4, (dplyr::n()-3):dplyr::n()) %>% 
  dplyr::pull(regulator) %>% 
  as.character()
```

```{r plot_dist_mats_cons_div}
dist <- dist_jk[paste0(top4_cons_div_modules, "_orig")]
names(dist) <- top4_cons_div_modules
plotDistMats(dist, ncol = 4)
```

```{r dist_mats_cons_div, eval = TRUE, include = TRUE, echo = FALSE, out.width = "100%", fig.align = 'center', fig.cap = 'The distance matrices of the 4 most conserved (top row) and 4 most diverged (bottom row) modules.'}
knitr::include_graphics("dist_mat_cons_div.png")
```

```{r plot_trees_cons_div}
trees <- trees_jk[paste0(top4_cons_div_modules, "_orig")]
names(trees) <- top4_cons_div_modules
plotTrees(trees, species_colors = spec_colors, ncol = 4)
```

```{r trees_cons_div, eval = TRUE, include = TRUE, echo = FALSE, out.width = "100%", fig.align = 'center', fig.cap = 'The tree reconstructions of the 4 most conserved (top row) and 4 most diverged (bottom row) modules.'}
...
These scores can be returned as a data frame using <tt>calculateEdgeDivergence</tt> or directly plotted using <tt>plotNetworks</tt> with <tt>color_by</tt> set to "edge_divergence". Below we plot the 300 strongest edges within each of the 4 most conserved and most diverged modules colored by edge divergence:

```{r}
plotNetworks(top4_cons_div_modules, 
             pruned_modules, 
             consensus_network, 
             network_list, 
             clone2species, 
...
```{r networks_cons_div, eval = TRUE, include = TRUE, echo = FALSE, out.width = "100%", fig.align = 'center', fig.cap = 'The 300 strongest connections of the 4 most conserved (top row) and 4 most diverged (bottom row) modules. The thickness of the edges represents the consensus edge weights and the color of the edges represents how different the mean edge weights are across the 3 species.'}
