...
#' }
#' @param tree Optional object of class \code{\link{phylo}}, the phylogenetic tree of the species (default: NULL).
...
comparePresStats <- function(pres_stats, random_pres_stats, tree = NULL, colors = NULL, font_size = 14) {
  ...
  if (!is.null(tree) && !inherits(tree, "phylo"))
    stop("The argument \"tree\" should be a phylo object, or NULL.")

  if (!is.null(tree)) {
    sp_in_data <- unique(c(as.character(pres_stats$species1), as.character(pres_stats$species2),
                           as.character(random_pres_stats$species1), as.character(random_pres_stats$species2)))
    if (any(!sp_in_data %in% tree$tip.label))
      stop("One or more species in the input data are not present in the phylogenetic tree. Please make sure species names match tree$tip.label.")
  }
  ...
  # differences between actual and random modules
  ...
  if (!is.null(tree)) {
    ...
    # calculate difference between within-species and cross-species scores
    pres_stats_to_plot <- pres_stats %>%
      tidyr::pivot_longer(cols = c("cor_adj", "cor_kIM"), names_to = "statistic", values_to = "value") %>%
      ...
      dplyr::group_by(dplyr::across(dplyr::any_of(c("regulator", "module_size", "statistic", "category")))) %>%
      dplyr::summarise(value = mean(.data[["value"]])) %>%
      dplyr::ungroup() %>%
      tidyr::pivot_wider(names_from = "category", values_from = "value") %>%
      dplyr::mutate(within_across_diff = .data[["within_species"]] - .data[["cross_species"]])

    # plot diff
    p2 <- pres_stats_to_plot %>%
      ggplot2::ggplot(ggplot2::aes(x = .data[["statistic"]], y = .data[["within_across_diff"]], fill = .data[["statistic"]])) +
      ggplot2::geom_boxplot(color = "grey20", linewidth = 0.1, outlier.alpha = 0.5, outlier.size = 0.2) +
