...
#'
#' Overall divergence is best represented by the total tree length, while species-specific divergence is best represented by the subtree length of a species, however the absolute values of these statistics in themselves are difficult to interpret. The diversity (either overall or for a particular species) can be used as an internal reference to determine whether the total tree length or the subtree length of a species is larger/smaller than expected. The general relationship between total tree length and overall diversity or between the subtree length and the diversity of the species is captured via linear regression (see also \code{\link{fitTreeStatsLm}}). In term of cross-species conservation, the most interesting modules are then the outliers that do not follow the linear trend.
#'
...
#' \item{weight}{Numeric, the weight of the module in the linear regression, inversely proportional to the variance of the total tree length/the subtree length of a species (only if \code{weighted_lm} is set to TRUE).}
#' \item{t_score}{Numeric, the t-score of the module. It is calculated as the residual normalized by the standard error of the total tree length/species subtree length prediction.}
#' \item{conservation}{Character, "not_significant" if the module falls inside the prediction interval of the fit, "diverged" if a module has a higher total tree length/subtree length of a species than the upper boundary of the prediction interval, and "conserved" if a module has a lower total tree length than the lower boundary of the prediction interval.}
#' }
#' @export
#'
