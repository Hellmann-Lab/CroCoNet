...
#'
#' Loads the TSV files containing the gene-gene edge weights and summarizes them as a list of \code{\link{igraph}} objects per replicate (i.e. cell line or biological replicate within a species).
#'
#' The functions reads and formats the TSV files in the directory specified by \code{path}. The base names of the TSV files of interest can be specified using the parameter \code{replicate_names} (in this case the files are expected to be named using the format: nameOfReplicate.tsv or nameOfReplicate_index.tsv). If  \code{replicate_names} is set to NULL (default), all TSV files in \code{path} are read and the replicate names are deduced from the file names by stripping anything starting with "_" and ".tsv".
#'
...
#'
#' If the edge weights were calculated on different subsamplings of cells per replicate, or a network inference algorithm involving stochastic steps (e.g. GRNBoost2) was run several times on each replicate, the edge weights are averaged across these subsamplings/runs, and a single combined \code{\link{igraph}} object is returned per replicate. The number of subsamplings/runs has to be specified by the parameter \code{rep}. The TSV files corresponding to the same replicate but different subsamplings/runs are expected to have the same base name with different indices: nameOfReplicate_index.tsv.
#'
...
#'
#' @return A named list of \code{\link{igraph}} objects containing the networks per replicate. The list names are taken from \code{replicate_names}. Each \code{\link{igraph}} object contains the edge attribute "weight" (taken from the 3rd column of the input TSV files). Starting from the 4th column of the input TSV files, all columns are preserved as edge attributes with unchanged names.
#' @export
#'
#' @examples
...
loadNetworks <- function(path, replicate_names = NULL, rep = 1L, directed = TRUE, min_occurrence = 2L, n_cores = 1L) {

  # check input data
  if (is.null(replicate_names))
    replicate_names <- unique(sapply(list.files(path), function(file) {strsplit(file, "_|.tsv")[[1]][1]}))
  ...
  if (!inherits(replicate_names, "character"))
    stop("The argument \"replicate_names\" should be a character vector.")
  ...
  . = replicate_name = i = gene1 = gene2 = rho = p.value = FDR = from = to = weight = n_supporting_edges = NULL

  # if the network inference was run only once per replicate
  if (rep == 1) {

    doParallel::registerDoParallel(n_cores)

    dt_list <- foreach::foreach(replicate_name = replicate_names) %dopar% {

      # load TSV files as data tables and rename columns
      dt <- data.table::fread(paste0(path, "/", replicate_name, ".tsv")) %>%
    ...
    names(dt_list) <- replicate_names

  # if the network inference was run several times per replicate
  } else {
    ...
    dt_list <- foreach::foreach(replicate_name = replicate_names) %dopar% {
      ...
      dt_all_reps <- lapply(1:rep, function(i) {
        ...
        if (file.exists(paste0(path, "/", replicate_name, "_", i, ".tsv")))
          data.table::fread(paste0(path, "/", replicate_name, "_", i, ".tsv")) %>%
          data.table::setnames(., 1:3, c("from", "to", "weight"))
