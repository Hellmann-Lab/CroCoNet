...
#'
#' Plots the distribution(s) of the chosen preservation statistic(s) for the actual and random modules and for within-species and cross-species replicate pairs. If a statistic is a good measure of module preservation, the preservation scores are expected to be 1) higher for the actual modules than for the random modules and 2) higher within species than across species.
#'
#' As part of the CroCoNet approach, pairwise module preservation scores are calculated between replicates, both within and across species (see \code{\link{calculatePresStats}}) to gain information about the cross-species differences but also about the within-species diversity of the modules. These correlation-based preservation statistics quantify how well the module topology is preserved between the networks of two replicates. The statistics are calculated not just for the actual, biologically meaningful modules, but also for random modules with matching sizes.
#'
...
#' \item{\{\{nameOfStat\}\}}{Numeric, one or more columns containing the preservation statistic(s) of interest per module and replicate pair.}
#' }
...
plotPresStatDistributions <- function(pres_stats, random_pres_stats, stats, colors = NULL, font_size = 14) {
  ...
  if (any(!c("regulator", "replicate1", "replicate2", "species1", "species2") %in% colnames(pres_stats)))
    stop("The argument \"pres_stats\" should contain the columns \"regulator\", \"replicate1\", \"replicate2\", \"species1\" and \"species2\".")

  if (!is.data.frame(random_pres_stats))
    stop("The argument \"random_pres_stats\" should be a data frame.")

  if (any(!c("regulator", "replicate1", "replicate2", "species1", "species2") %in% colnames(random_pres_stats)))
    stop("The argument \"random_pres_stats\" should contain the columns \"regulator\", \"replicate1\", \"replicate2\", \"species1\" and \"species2\".")
...
#'
#' For example, if there are 2 species, human and gorilla, with 3 human replicates (\emph{h1}, \emph{h2} and \emph{h3}), and 2 gorilla replicates (\emph{g1} and \emph{g2}), for a single module there will be 3 preservation scores within the human replicates (\eqn{p_{h1-h2}}, \eqn{p_{h1-h3}} and \eqn{p_{h2-h3}}), 1 preservation score within the gorilla replicates (\eqn{p_{g1-g2}}) and 6 preservation scores across the 2 species (\eqn{p_{h1-g1}}, \eqn{p_{h1-g2}}, \eqn{p_{h2-g1}}, \eqn{p_{h2-g2}}, \eqn{p_{h3-g1}} and \eqn{p_{h3-g2}}). For this module and the human-gorilla species pair, the summarized within-species score will then be the mean of the 4 within-species scores (\eqn{mean(p_{h1-h2}, p_{h1-h3}, p_{h2-h3}, p_{g1-g2})}) and the cross-species score will be the mean of the 6 cross-species scores (\eqn{mean(p_{h1-g1}, p_{h1-g2}, p_{h2-g1}, p_{h2-g2}, p_{h3-g1}, p_{h3-g2})}).
#'
...
#' \item{replicate1, replicate2}{Character, the names of the replicates compared.}
#' \item{species1, species2}{Character, the names of the species \code{replicate1} and \code{replicate2} belongs to, respectively.}
#' \item{\{\{nameOfStat\}\}}{Numeric, one or more columns containing the preservation statistic of interest per module and replicate pair.}
#' }
