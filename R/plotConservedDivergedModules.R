...
#' \item{t_score}{Numeric, the t-score of the module. It is calculated as the residual normalized by the standard error of the species subtree length prediction.}
#' \item{conservation}{Character, 'not_significant' if the module falls inside the prediction interval of the fit, 'diverged' if a module has a higher species subtree length than the upper boundary of the prediction interval, and 'conserved' if a module has a lower species subtree length than the lower boundary of the prediction interval.}
#' }
...
plotConservedDivergedModules <- function(module_conservation, N = 5L, rank_by = "residual", colors = NULL, font_size = 14, label_size = 2.5) {
  ...
  if (focus == "overall") {
    ...
    p <- module_conservation %>%
      ...
      ggplot2::geom_point(ggplot2::aes(color = .data[["conservation"]], size = .data[["conservation"]])) +
      ggplot2::theme_bw(base_size = font_size) +
      ggplot2::scale_color_manual(values = colors, breaks = "diverged", labels = paste0("diverged on the\n", focus, " lineage")) +
    ...
    if (sum(c(paste0("lwr_", focus, "_subtree_length"), paste0("upr_", focus, "_subtree_length"), paste0("lwr_", focus, "_diversity"), paste0("upr_", focus, "_diversity")) %in% colnames(module_conservation)) == 4) {
      ...
      p <- p +
        ...
        ggplot2::geom_errorbar(ggplot2::aes(ymin = .data[[paste0("lwr_", focus, "_subtree_length")]], ymax = .data[[paste0("upr_", focus, "_subtree_length")]], color = .data[["conservation"]]), linewidth = 0.2)
