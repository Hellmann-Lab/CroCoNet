...
#'
#' @param network_list A named list that contains the networks of each replicate and the consensus network in an \code{\link{igraph}} format.
...
testPowers <- function(network_list, powers = 1:20, n_cores = 1L) {
  ...
  scale_free_fit <- foreach::foreach(power = powers,
                                     .combine = rbind) %:%
    foreach::foreach(replicate_name = replicate_names,
                     .combine = c) %dopar% {

                       # get connectivities
                       network <- network_list[[replicate_name]]
  ...
  colnames(scale_free_fit) <- replicate_names
...
#' @param n_cores Integer, the number of cores (default: 1).
#'
#' @return A named list of \code{\link{igraph}} objects containing the networks of all replicates after the scale-free transformation on the edge weights
#' @export
#'
