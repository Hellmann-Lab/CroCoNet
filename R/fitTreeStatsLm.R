...
fitTreeStatsLm <- function(tree_stats, focus = "overall", weighted_lm = TRUE) {
  ...
  if (focus == "overall") {
    ...
    if (any(!(c(paste0(focus, "_diversity"), paste0(focus, "_to_other_branch_length")) %in% colnames(tree_stats))))
      stop(paste0("The argument \"tree_stats\" should contain the columns corresponding to \"focus\". If \"focus\" is set to \"", focus, "\", the required columns are \"", focus, "_diversity\" and \"", focus, "_to_other_branch_length\"."))

    if (weighted_lm & !paste0("var_", focus, "_to_other_branch_length") %in% colnames(tree_stats))
      stop(paste0("If \"focus\" is set to \"", focus, "\" and \"weighted_lm\" is set to TRUE, \"tree_stats\" is expected to contain the column \"var_", focus, "_to_other_branch_length\". Please add this column to the data frame or change the value of \"weighted_lm\" or \"focus\"."))
  ...
  if (focus == "overall") {
    ...
    if (weighted_lm) {
      ...
      # get weights
      tree_stats <- tree_stats %>%
        dplyr::mutate(weight = 1 / .data[[paste0(focus, "_to_other_branch_length")]],
                      weight = .data[["weight"]] / sum(.data[["weight"]]))

      # fit weighted lm
      lm_tree_stats <- stats::lm(stats::as.formula(paste0(focus, "_to_other_branch_length ~ ", focus, "_diversity")), tree_stats, weights = weight)

    } else {

      # fit weighted lm
      lm_tree_stats <- stats::lm(stats::as.formula(paste0(focus, "_to_other_branch_length ~ ", focus, "_diversity")), tree_stats)
