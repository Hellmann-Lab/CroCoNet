...
plotConservedDivergedTargets <- function(target_conservation, N = 5, label = "auto", colors = NULL, font_size = 14) {
  ...
  if (focus == "overall") {
    ...
    if (any(!(c(paste0(focus, "_diversity"), paste0(focus, "_to_other_branch_length")) %in% colnames(target_conservation))))
      stop(paste0("The argument \"target_conservation\" should contain the columns corresponding to the value of the column \"focus\". If \"focus\" is \"", focus, "\", the required columns are \"", focus, "_diversity\" and \"", focus, "_to_other_branch_length\"."))
