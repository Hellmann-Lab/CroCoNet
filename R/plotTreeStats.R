...
#' \item{\{\{nameOfStat\}\}}{Numeric, one or more columns containing the tree statistic of interest per module and replicate pair.}
#' }
#' @param random_tree_stats Data frame of the tree statistics for the random modules. Required columns:
#' \describe{
#' \item{regulator}{Character, transcriptional regulator.}
#' \item{\{\{nameOfStat\}\}}{Numeric, one or more columns containing the tree statistics of interest per module and replicate pair.}
#' }
...
#'
#' As part of the CroCoNet approach, pairwise module preservation scores are calculated between replicates, both within and across species (see \code{\link{calculatePresStats}}) and neighbor-joining trees are reconstructed based on these preservation scores per module (see \code{\link{convertPresToDist}} and \code{\link{reconstructTrees}}). The tips of the resulting tree represent the replicates and the branch lengths represent the dissimilarity of module topology between the networks of 2 replicates.  Various statistics can be defined based on these trees such as total tree length, overall within-species diversity, and the subtree length and diversity of each species (see \code{\link{calculateTreeStats}}).
#'
#' These tree-based statistics can be used to pinpoint conserved and diverged modules. First, a linear regression model is fit between the total tree length and within-species diversity (in case the focus of interest is conservation and overall divergence) or between the subtree length and the diversity of a species (in case the focus of interest is the species-specific divergence), and then outlier data points are identified that do not follow the general linear trend (see \code{\link{fitTreeStatsLm}} and \code{\link{findConservedDivergedModules}}).
#'
#' This function visualizes the relationship of 2 tree-based statistics. This can be an informative check for the pairs of statistics that are used to measure cross-species conservation - total-tree length VS within-species diversity (default) or the subtree length VS the diversity of a species - before fitting a linear regression model between them.
#'
...
#' \item{\{\{nameOfStat\}\}}{Numeric, 2 columns containing the tree statistics to be plotted against each other.}
#' }
#' @param stats Character vector of length 2, the names of the columns containing the 2 statistics that should be plotted against each other (default: c("within_species_diversity", "total_tree_length"), another useful option is to plot the subtree length and the diversity of a species against each other). The 1st element of the vector will be plotted on the x-axis and the 2nd element of the vector will be plotted on the y-axis.
